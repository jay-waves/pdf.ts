...
import type { ViewerCommand, ViewerCommandDispatch } from '../viewer/viewer-controller';
...
export function installPageNavigationInput(pages: PageController, dispatch: ViewerCommandDispatch) {
  const onNavigate = (delta: number, source: 'Keyboard' | 'Mouse') => {
    dispatch({ type: 'navigation/move-pages', delta, source });
  };
  const onKeyDown = (event: KeyboardEvent) => {
    if (event.defaultPrevented || event.altKey || event.ctrlKey || event.metaKey) return;
    if (!isViewerNavigationTarget(event.target)) return;
    const presenting = pages.getSnapshot().mode === 'presentation';
