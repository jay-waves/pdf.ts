...
import { Dialog, PanelContent } from './components';
import {
    ...
    >
      <PanelContent className={styles.content}>
        ...
        <section className={styles.section} aria-labelledby="developer-controls">
          <h2 id="developer-controls" className={styles.sectionTitle}>Rendering controls</h2>
          <label className={styles.control}>
            <span>Device Pixel Ratio (DPR)</span>
            <select
              className={styles.select}
              value={dprMode}
              onChange={(event) => onDprModeChange(event.currentTarget.value as RenderDprMode)}
            >
          ...
          <p className={styles.hint}>
            Each profile also adjusts tile raster scale; tile edges follow a fixed {PDF_TILE_SIZE_CSS_PX} CSS px × DPR ratio.
          </p>
        </section>

        <label className={styles.detailsLabel}>
          <span>Details</span>
          <textarea className={styles.details} value={details} readOnly rows={9} />
        </label>
      </PanelContent>
    </Dialog>
  );
