...
const {
  ...
  renderSettingsStore,
  setRenderDprMode,
} = await import('../apps/renderer/render-settings.ts');

test('DPR preferences survive module reloads', async () => {
  for (const mode of ['1.25', '1.5', '1.75', 'system', 'auto']) {
    setRenderDprMode(mode);
    assert.equal(renderSettingsStore.getState().dprMode, mode);
    const restored = await import(`../apps/renderer/render-settings.ts?mode=${mode}`);
    assert.equal(restored.renderSettingsStore.getState().dprMode, mode);
  }
});
...
test('missing or invalid DPR preferences fall back to auto', async () => {
  for (const value of [null, '', '3', 'invalid']) {
    ...
    const restored = await import(`../apps/renderer/render-settings.ts?fallback=${value}`);
    assert.equal(restored.renderSettingsStore.getState().dprMode, 'auto');
  }
});
...
test('DPR monitor follows display changes without overriding the browser property', () => {
  ...
  assert.equal(renderSettingsStore.getState().systemDpr, 1.25);
